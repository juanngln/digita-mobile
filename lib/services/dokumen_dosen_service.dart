...
class DokumenDosenService {
  ...
  Future<List<DocumentDetails>> fetchDokumenMahasiswa(int mahasiswaId) async {
    ...
    final response = await http.get(
      Uri.parse('${AppConfig
          .baseUrl}/api/v1/tugas-akhir/dokumen/?mahasiswa_id=$mahasiswaId'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
  ...
  Future<DocumentDetails> updateDokumenStatus({
    ...
    final url = Uri.parse(
        '${AppConfig
            .baseUrl}/api/v1/tugas-akhir/dokumen/$dokumenId/update-status/');
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
          'Gagal memperbarui status dokumen: ${errorBody['detail'] ??
  ...
  Future<String> getAccessFileUrl(int dokumenId) async {
    ...
    final url = Uri.parse('${AppConfig
        .baseUrl}/api/v1/tugas-akhir/dokumen/$dokumenId/access-file/');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw NetworkException(
            "Gagal mendapatkan akses file. Status: ${response
                .statusCode}, Pesan: ${response.body}");
      }
    } on SocketException {
      throw NetworkException(
  ...
  Future<String> getDownloadFileUrl(int dokumenId) async {
    ...
    final url = Uri.parse('${AppConfig
        .baseUrl}/api/v1/tugas-akhir/dokumen/$dokumenId/access-file/?action=download');
